# ParParty — AWS Deployment Guide

Step-by-step checklist for deploying the Dev and Prod stacks using AWS CDK.

---

## Prerequisites

### 1. AWS CLI configured
```bash
aws configure
# Enter: Access Key ID, Secret Access Key, default region (us-east-1), output format (json)

# Verify it's working
aws sts get-caller-identity
```

### 2. Bootstrap CDK (one-time per account/region)
```bash
cd parparty-cdk
npx cdk bootstrap aws://YOUR_ACCOUNT_ID/us-east-1
```
Find your account ID with: `aws sts get-caller-identity --query Account --output text`

### 3. Docker running
CDK builds container images from source (`parparty-api`, `parparty-web`, `parparty-server`) and pushes them to ECR. Docker must be running on your machine at deploy time.

---

## Dev Stack (`ParPartyDevStack`)

### Step 1 — Deploy
```bash
cd parparty-cdk
npx cdk deploy ParPartyDevStack
```
Note the `AppUrl` output — this is the ALB DNS name (e.g. `http://ParParty-Alb-xxxx.us-east-1.elb.amazonaws.com`).

### Step 2 — Update `appBaseUrl`
Open `parparty-cdk/bin/parparty-cdk.ts` and update line 31:
```ts
appBaseUrl: 'http://<ALB-DNS-NAME-FROM-OUTPUT>',
```
Then redeploy: `npx cdk deploy ParPartyDevStack`

This URL is embedded in email invite links, so it must point to where the app is actually running.

### Step 3 — Populate Gmail SMTP credentials
Generate a Gmail App Password: Google Account → Security → 2-Step Verification → App passwords

```bash
aws secretsmanager put-secret-value \
  --secret-id parparty-dev/gmail-smtp-credentials \
  --secret-string '{"username":"you@gmail.com","password":"xxxx xxxx xxxx xxxx"}'
```

### Step 4 — Populate optional secrets
```bash
# OpenAI (AI assistant feature)
aws secretsmanager put-secret-value \
  --secret-id parparty-dev/openai-api-key \
  --secret-string 'sk-...'

# Stripe test keys (payment feature)
aws secretsmanager put-secret-value \
  --secret-id parparty-dev/stripe-secret-key \
  --secret-string 'sk_test_...'

aws secretsmanager put-secret-value \
  --secret-id parparty-dev/stripe-publishable-key \
  --secret-string 'pk_test_...'
```

> **Auto-generated — no action needed:**
> `parparty-dev/db-credentials` and `parparty-dev/jwt-secret` are generated by CDK automatically.

---

## Prod Stack (`ParPartyProdStack`)

### Step 1 — Create a Route53 hosted zone for `parparty.com`
If `parparty.com` is not already in Route53, create a hosted zone:
- AWS Console → Route53 → Hosted Zones → Create hosted zone → `parparty.com`
- Update your domain registrar's nameservers to match the NS records Route53 provides.

Find the hosted zone ID:
```bash
aws route53 list-hosted-zones --query "HostedZones[?Name=='parparty.com.'].Id" --output text
# Returns something like: /hostedzone/Z1234ABCDE5678
# The ID is the part after /hostedzone/
```

### Step 2 — Request an ACM certificate for HTTPS
The ALB requires an ACM certificate to terminate TLS on port 443.

```bash
# Request a certificate covering the apex and www subdomain
aws acm request-certificate \
  --domain-name parparty.com \
  --subject-alternative-names '*.parparty.com' \
  --validation-method DNS \
  --region us-east-1

# Get the certificate ARN (note it — you'll need it in the next step)
aws acm list-certificates \
  --query "CertificateSummaryList[?DomainName=='parparty.com'].CertificateArn" \
  --output text \
  --region us-east-1
```

ACM will provide CNAME records for DNS validation. Add them to your Route53 hosted zone:
- AWS Console → Certificate Manager → the new cert → "Create records in Route53" button
- Or add the CNAME records manually in Route53

Wait for the certificate status to show **Issued** before deploying (usually 1–5 minutes after DNS records propagate).

### Step 3 — Set env vars and deploy
```bash
export PARPARTY_HOSTED_ZONE_ID=Z1234ABCDE5678
export PARPARTY_CERT_ARN=arn:aws:acm:us-east-1:123456789012:certificate/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
cd parparty-cdk
npx cdk deploy ParPartyProdStack
```

This creates:
- Route53 A alias records: `parparty.com` and `www.parparty.com` → ALB
- ALB HTTPS listener on port 443 (TLS terminated using the ACM cert)
- ALB HTTP listener on port 80 → permanent redirect to HTTPS
- SES email identity for `parparty.com` (outputs DNS verification records)

### Step 4 — Verify SES domain ownership
After deploy, CDK outputs CNAME/TXT records for SES domain verification. Add them to your Route53 hosted zone:
- AWS Console → Route53 → Hosted Zones → `parparty.com` → Create record
- Or use the AWS Console → SES → Verified Identities → `parparty.com` → view the records

Wait for SES to show the domain as **Verified** before expecting emails to send.

### Step 5 — Populate SES SMTP credentials
Generate SES SMTP credentials: AWS Console → SES → SMTP Settings → Create SMTP credentials (creates an IAM user and derives SMTP password from the access key).

```bash
aws secretsmanager put-secret-value \
  --secret-id parparty-prod/ses-smtp-credentials \
  --secret-string '{"username":"AKIAIOSFODNN7EXAMPLE","password":"<derived-smtp-password>"}'
```

### Step 6 — Populate optional secrets
```bash
# OpenAI (AI assistant feature)
aws secretsmanager put-secret-value \
  --secret-id parparty-prod/openai-api-key \
  --secret-string 'sk-...'

# Stripe live keys (payment feature)
aws secretsmanager put-secret-value \
  --secret-id parparty-prod/stripe-secret-key \
  --secret-string 'sk_live_...'

aws secretsmanager put-secret-value \
  --secret-id parparty-prod/stripe-publishable-key \
  --secret-string 'pk_live_...'
```

> **Auto-generated — no action needed:**
> `parparty-prod/db-credentials` and `parparty-prod/jwt-secret` are generated by CDK automatically.

### Step 7 — Request SES production access (if needed)
By default, SES accounts are in sandbox mode and can only send to verified addresses. To send to any address:
- AWS Console → SES → Account dashboard → Request production access
- Fill out the form explaining your use case (transactional email for a golf app)

---

## Quick Reference — All Secrets

| Secret Name | Who sets it | Purpose |
|---|---|---|
| `parparty-dev/db-credentials` | CDK (auto) | RDS MySQL password |
| `parparty-dev/jwt-secret` | CDK (auto) | JWT signing key |
| `parparty-dev/gmail-smtp-credentials` | You (Step 3) | Gmail app password for dev email |
| `parparty-dev/openai-api-key` | You (Step 4) | OpenAI API key |
| `parparty-dev/stripe-secret-key` | You (Step 4) | Stripe test secret key |
| `parparty-dev/stripe-publishable-key` | You (Step 4) | Stripe test publishable key |
| `parparty-prod/db-credentials` | CDK (auto) | RDS MySQL password |
| `parparty-prod/jwt-secret` | CDK (auto) | JWT signing key |
| `parparty-prod/ses-smtp-credentials` | You (Step 5) | SES SMTP credentials for prod email |
| `parparty-prod/openai-api-key` | You (Step 6) | OpenAI API key |
| `parparty-prod/stripe-secret-key` | You (Step 6) | Stripe live secret key |
| `parparty-prod/stripe-publishable-key` | You (Step 6) | Stripe live publishable key |

---

## Accessing Camunda (internal service)

Camunda is not exposed publicly. Use ECS Exec to open a shell into the running task:

```bash
# Dev
aws ecs execute-command --cluster parparty-dev \
  --task <task-id> --interactive --command "/bin/sh"

# Prod
aws ecs execute-command --cluster parparty-prod \
  --task <task-id> --interactive --command "/bin/sh"
```

Find the task ID: AWS Console → ECS → Clusters → `parparty-dev` → Tasks

---

## Tearing Down

```bash
# Dev (safe to destroy — no deletion protection)
npx cdk destroy ParPartyDevStack

# Prod (deletion protection is ON on RDS — must disable first)
# AWS Console → RDS → Modify instance → uncheck deletion protection → apply immediately
npx cdk destroy ParPartyProdStack
```
